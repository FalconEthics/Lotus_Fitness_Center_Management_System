                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                        {STATUS_OPTIONS.map((option) => {
                            ...
                            return (
                                <label
                                    key={option.value}
                                    className={`cursor-pointer p-4 rounded-lg border-2 transition-all ${formData.status === option.value
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="max-h-64 overflow-y-auto border border-base-300 rounded-lg">
                            {(formData.classId ? enrolledMembers : filteredMembers).map(member => (
                                <label
                                    key={member.id}
                                    className={`flex items-center gap-3 p-3 cursor-pointer hover:bg-base-200 border-b border-base-300 last:border-b-0 ${selectedMembers.includes(member.id) ? 'bg-primary/5' : ''
                                        }`}
                                >
                                    <input
                                        type="checkbox"
